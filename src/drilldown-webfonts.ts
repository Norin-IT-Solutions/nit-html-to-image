...
const perfMeasure = (name: string, start: string, end: string) => {
    ...
    if (entry) console.log(`${name}: ${Math.round(entry.duration)}ms`);
...
const urlToDataUrl = async (url: string): Promise<string> => {
    ...
    performance.mark('font-fetch-start');
    ...
    performance.mark('font-fetch-end');
    perfMeasure(`font fetch ${url}`, 'font-fetch-start', 'font-fetch-end');
...
const collectUsedTriples = (root: HTMLElement): Set<Triple> => {
    ...
    for (let el = walker.currentNode as HTMLElement; el; el = walker.nextNode() as HTMLElement) {
        const cs = getComputedStyle(el);
        const families = cs.fontFamily.split(',').map((s) => s.trim().replace(/^['"]|['"]$/g, ''));
        const family = families[0];
        if (!family || /^(serif|sans-serif|monospace|system-ui|ui-)/i.test(family)) continue;
    ...
    perfMeasure('collectUsedTriples', 'collect-start', 'collect-end');
    return triples;
};
...
const getMatchingFontFaceRules = (triples: Set<Triple>): CSSFontFaceRule[] => {
    ...
    for (const sheet of Array.from(document.styleSheets)) {
        ...
        for (const rule of Array.from(rules)) {
            ...
            const family = ff.style.getPropertyValue('font-family').trim().replace(/^['"]|['"]$/g, '');
            if (!family) continue;
            const style = (ff.style.getPropertyValue('font-style') || 'normal').trim() || 'normal';
    ...
    perfMeasure('getMatchingFontFaceRules', 'match-start', 'match-end');
    return out;
};

// 3) inline one @font-face’s src to data URL (memoized)
const inlineFontFace = async (rule: CSSFontFaceRule): Promise<string | null> => {
    ...
    ).map((m) => ({ url: m[1].replace(/^['"]|['"]$/g, ''), format: m[2].toLowerCase() }));

    const pick = urls.find((u) => u.format.includes('woff2')) ?? urls[0];
    if (!pick) return null;

    performance.mark('inline-face-start');
    ...
    performance.mark('inline-face-end');
    perfMeasure(`inlineFontFace ${family} ${weight} ${style}`, 'inline-face-start', 'inline-face-end');
...
export const buildFontEmbedCSS = async (root: HTMLElement): Promise<string> => {
    performance.mark('build-embed-start');

    const triples = collectUsedTriples(root);
    if (!triples.size) {
        performance.mark('build-embed-end');
        perfMeasure('buildFontEmbedCSS(empty)', 'build-embed-start', 'build-embed-end');
        return '';
    }
    ...
    if (cached) {
        performance.mark('build-embed-end');
        perfMeasure('buildFontEmbedCSS(cache hit)', 'build-embed-start', 'build-embed-end');
        return cached;
    }
    ...
    performance.mark('build-embed-end');
    perfMeasure('buildFontEmbedCSS', 'build-embed-start', 'build-embed-end');
